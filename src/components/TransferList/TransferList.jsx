...
const TransferList = ({users, clients, architectures, archiSelected, clientSelected, circlesOfSelectedArchi}) => {

    const [tabDataCircles, settabDataCircles] = useState()
    ...
    useEffect(()=>{
        ...
        //     let circleList = []
        //     circlesOfSelectedArchi.forEach((circle, i)=>{
    ...
    useEffect(()=>{
        ...
        let tabAdmins = admins.map((admin)=>{
            return users.find(e=>e._id === admin)
        })

        let tabOperators = operators.map((operator)=>{
            return users.find(e=>e._id === operator)
        })

        let tabAccess = access.map((access)=>{
            return users.find(e=>e._id === access)
        })
    ...
    return (
        ...
        <div className="list-container" style={{height: '100%', width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', boxSizing: 'border-box', paddingTop: 20}}>
            ...
            users.map((person, i)=>{
                ...
                return <div onClick={()=>{toggleSelectionPeople(person.id); toggleSelectionCircle(null)}} style={currentSelectionPeople === person.id ? styles.itemPeopleSelected : styles.itemPeople}>
        ...
        <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-around', alignItems: 'center'}}>
            <h3 style={(currentSelectionCircle || currentSelectionPeople) && combineStyles(theme.foreground,{textAlign: 'center', padding: '0px 10px 10px 10px', borderBottom: '1px solid rgba(0,0,0,0.6)', color: 'rgba(0,0,0,0.6)', position: 'absolute', top: '23%', overflow: 'visible'})}>{currentSelectionCircle && tabDataCircles.find(e=>e._id === currentSelectionCircle).name}{currentSelectionPeople && users.find(e=>e._id === currentSelectionPeople).firstname} {currentSelectionPeople && users.find(e=>e._id === currentSelectionPeople).lastname}</h3>
